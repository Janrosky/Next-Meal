...
test('kiosk, cash payment, live kitchen, close shift and admin reports', async ({ browser }) => {
  ...
  await customer.screenshot({ path: '../docs/screenshots/autoservicio.png', fullPage: true });
  ...
  await expect(customer.locator('.ticket-number')).toContainText('#0001');
  ...
  await expect(cashier.getByRole('heading', { name: 'Caja abierta · #1' })).toBeVisible();
  ...
  await kitchen.screenshot({ path: '../docs/screenshots/cocina.png', fullPage: true });
  ...
  await expect(cashier.getByText('Cuadra', { exact: true })).toBeVisible();

  await login(admin, 'admin');
  await expect(admin.locator('.metric').first()).toContainText('4');
  await admin.screenshot({ path: '../docs/screenshots/administracion.png', fullPage: true });
  ...
  await customer.screenshot({ path: '../docs/screenshots/movil.png', fullPage: true });
